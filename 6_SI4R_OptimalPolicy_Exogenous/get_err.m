function [err,I,Ia0,Ia1,Ib0,Ib1,S,R,R0,R1,D,T,Pop,RnotSIRmacro,aggC,aggH,ns,nia0,nia1,nib0,nib1,nr0,nr1,cs,cia0,cia1,cib0,cib1,cr0,cr1,Us,Uia0,Uia1,Uib0,Uib1,Ur0,Ur1,U,probSD,probA0D,probR0D] = get_err(guess,A,theta,i_ini,pop_ini,pis1,pis2,pis3,pir,pid,betta,Uib1ss,Uia1ss,HH,C,N,Ur1ss,muc,phii,pia,pib,pit)
...
lambs=(cs.^(-1)+lamtaus*pis1.*I(1:HH).*cib0)./(1+muc);
...
lambia0=(cia0.^(-1)+lamtauia0*pis1.*I(1:HH).*cib0)./(1+muc);
...
lambr0=(cr0.^(-1)+lamtaur0*pis1.*I(1:HH).*cib0)./(1+muc);

%equation residuals - Gradient Adjusting Method Extended for SI4R.
err=ones(6*HH,1);
err(1:HH)=(1+muc).*cia0-A.*nia0-Gamma;
err(HH+1:2*HH)=(1+muc).*cia1-A.*nia1-Gamma;
err(2*HH+1:3*HH)=(1+muc).*cib0-phii*A.*nib0-Gamma;
err(3*HH+1:4*HH)=(1+muc).*cib1-phii*A.*nib1-Gamma;
err(4*HH+1:5*HH)=muc.*(S(1:HH).*cs+(Ia0(1:HH).*cia0+Ia1(1:HH).*cia1+Ib0(1:HH).*cib0+Ib1(1:HH).*cib1)+R1(1:HH).*cr0+R0(1:HH).*cr1)-Gamma.*(S(1:HH)+Ia0(1:HH)+Ia1(1:HH)+Ib0(1:HH)+Ib1(1:HH)+R0(1:HH)+R1(1:HH));
