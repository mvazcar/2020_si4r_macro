...
sirmacro=0;         % =1 it computes SIR-Macro instead of SI4R.
optimalexogenous=1; % =1 sets SIR-Macro optimal muc1 as exogenous path for muc1.
...
muc0=zeros(HH,1); 
muc1=zeros(HH,1);    %exogenous path for muc1 over time.
                    %if you want e.g. a containment policy of
                    %10 percent for weeks 1,...52, then set muc1(1:52)=0.1;
                    %
                    %Make sure that muc1=0 at the end of the solution and
                    %simulation horizon. Steady state assumes muc1=0;
                    %
                    %With optimal policy, muc1 path will be chosen to
                    %maximize PV utility (switch below).
                    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SI4R Extra Parameters
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if optimalexogenous==1 % Use Optimal muc1 of SIR Macro in SI4R as exogenous path for muc1.
load last_solution_opt_policy; 
muc1=muc_sol;    
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
do_opt_policy=0;    %switch: if = 0, model is solved and simulated with
                    %given path for containment policy muc1
                    %
                    %if =1, model is solved and simulated with optimal
                    %containment path muc1. Path for muc1 set above will be
...
%If optimal policy is desired, find optimal path for muc1 to maximize PV utility
if do_opt_policy==1 %optimal policy
    
    %minimize negative PV utility (i.e. max PV utility) to find opt. path for muc1; nonlinear
    %model equations are solved inside getU.m using function get_err.m
    LB=muc1_guess*0-2;UB=muc1_guess*0+2;%lower and upper bounds
   
    muc1_sol = fmincon(@getU,muc1_guess,[],[],[],[],LB,UB,[],opts_fmincon,n_vec_guess,opts_fsolve_fmincon,A,theta,i_ini,pop_ini,pis1,pis2,pis3,pir,pid,betta,Uib1ss,Uia1ss,HH,cr1ss,nr1ss,Ur1ss,phii,pia,pib,pit);

    muc1=zeros(HH,1);muc1(1:numel(muc1_sol))=muc1_sol;
    save last_solution_opt_policy muc1_sol;%save solution for possible use in subsequent maximization   
end
 
%Given either optimal path for muc1 or exogenous path for muc1,
%solve nonlinear equilibrium model equations (i.e. adjust guesses ns,nia0,nia1,nib0,nib1,nr0,nr1)
[n_vec,fval,exitflag]=fsolve(@get_err,n_vec_guess,opts_fsolve,A,theta,i_ini,pop_ini,pis1,pis2,pis3,pir,pid,betta,Uib1ss,Uia1ss,HH,cr1ss,nr1ss,Ur1ss,muc0,muc1,phii,pia,pib,pit);
...
%get allocations given either exogenous or optimal path for muc1 at ns,nia0,nia1,nib0,nib1,nr0,nr1
%solution
